...
class OnlyAwardOrRelatedValidator:
    """
    Ограничение по выбору одного из значений: связанной привычки или вознаграждения.
    """
    ...
    def __call__(self, value):
        if dict(value).get("related_habit") and dict(value).get("award"):
            raise serializers.ValidationError("Можно заполнить либо поле Вознаграждение, либо поле Связанная привычка")
...
class PeriodValidator:
    ...
    def __call__(self, value):
        ...
        if period != 'daily' and period != 'weekly':
            raise serializers.ValidationError('Нельзя выполнять привычку реже одного раза в неделю')
